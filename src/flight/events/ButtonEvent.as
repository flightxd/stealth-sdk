...
package flight.events
{
	...
	public class ButtonEvent extends MouseEvent
	{
		...
		{
			if (button) {
				button.addEventListener(MouseEvent.MOUSE_DOWN,	onMouseDown,	false, 10, true);
				button.addEventListener(MouseEvent.ROLL_OVER,	onRollOver,		false, 10, true);
				button.addEventListener(MouseEvent.ROLL_OUT,	onRollOut,		false, 10, true);
				button.addEventListener(MouseEvent.MOUSE_UP,	onMouseUp,		false, 10, true);
				
				if (includeCallbacks) {
					
					button.addEventListener(MouseEvent.CLICK,			onCallbackEvent, false, 10, true);
					button.addEventListener(MouseEvent.DOUBLE_CLICK,	onCallbackEvent, false, 10, true);
					button.addEventListener(MouseEvent.ROLL_OVER,		onCallbackEvent, false, 10, true);
					button.addEventListener(MouseEvent.ROLL_OUT,		onCallbackEvent, false, 10, true);
					button.addEventListener(PRESS,						onCallbackEvent, false, 10, true);
					button.addEventListener(DRAG,						onCallbackEvent, false, 10, true);
					button.addEventListener(DRAG_OVER,					onCallbackEvent, false, 10, true);
					button.addEventListener(DRAG_OUT,					onCallbackEvent, false, 10, true);
		...
		private static function onMouseDown(event:MouseEvent):void
		{
			...
			button.stage.addEventListener(MouseEvent.MOUSE_MOVE, onMouseMove, false, 10, true);
			button.stage.addEventListener(MouseEvent.MOUSE_UP, onRelease, false, 10, true);
			button.stage.addEventListener(Event.MOUSE_LEAVE, onRelease, false, 10, true);
			
			pressedIndex[button] = 1;
			holdIntervals[button] = setTimeout(onDelayHold, DELAY_INTERVAL, button);		// TODO: don't use setTimeout, it's a crock! (creates Timer's under the hood)
		...
		private static function onDelayHold(button:InteractiveObject):void
		{
			...
			holdIntervals[button] = setInterval(onHold, HOLD_INTERVAL, button);		// TODO: don't use setInterval, it's a crock! (creates Timer's under the hood)
		}
